...
void					get_new_p(t_vec2d *new, t_grid *grid, t_world *world)
{
	if (!world || !grid || !new || !within(new[0].x, -1, GRID_SIZE) ||
		...
		return ;
	if (node_in_sec(new[0], world) != -1 ||
		grid->nodes[new[0].x][new[0].y] != NODE_EMPTY ||
		find_wall_inter(new[0], *world) != -1)
		new[0] = (t_vec2d){ -1, -1 };
	if (node_in_sec(new[1], world) != -1 ||
		grid->nodes[new[1].x][new[1].y] != NODE_EMPTY ||
		find_wall_inter(new[1], *world) != -1)
		new[1] = (t_vec2d){ -1, -1 };
}

unsigned short			will_inter(t_world *world, t_vec2d one, t_vec2d two)
{
	unsigned short		i;
	int					j;
	t_vec2d				v1;
...
void					get_door_points(t_vec2d *p, t_grid *grid, \
...
void					add_door(t_world *w, int wall, t_grid *g)
{
	...
	get_door_points((t_vec2d *)p, g, w, get_perp_slope(p[0], p[1]));
	...
	== SUCCESS && add_wall(&w->walls, (w->sec[w->n_s].s_walls[2] = w->n_w++),\
