																...
																t_prog *prog)
{
	...
	if (!sdl || !m || !grid || !prog)
	{
		printf("returning fail from update editor, condition 1\n");
		return (FAIL);
	}

	if (prog->last != prog->m_id)
	{
		int tmp2 = mode_change(prog, m, grid, select_it(1, FC_SELECT, -1));
		if (tmp2 == FAIL)
			printf("returning fail from update editor, condition 2\n");
		return (tmp2);
	}
	if (prog->zoom != 0)
	...
	{
		int tmp2 = move_grid_keys(prog, grid);
		if (tmp2 == FAIL)
			printf("returning fail from update editor, condition 3\n");
		return (tmp2);
	}
	if (select_it(1, ST_SELECT, -1) == NORMAL && prog->btn_on == SAVE_BTN)
	{
		int tmp2 = save_media(m, prog);
		if (tmp2 == FAIL)
			printf("returning fail from update editor, condition 4\n");
		return (tmp2);
	}
	if ((tmp = manage_btn(m, prog, grid, sdl->mouse)) < 2)
	{
		if (tmp == FAIL)
			printf("returning fail from update editor, condition 5\n");
		return (tmp);
	}
		...
		t_prog *prog)
{
	...
	while(SDL_PollEvent(&e))
	{
		...
		{
			printf("returning quit=true from get input\n");
			return (TRUE);
		}
