...
unsigned				read_line(char *str, unsigned short status, t_world *world, unsigned short world_no)
{
	...
	{
		p = (t_vec){ clamp(p.x, 0, GRID_SIZE), clamp(p.y, 0, GRID_SIZE) };
	...
	if (status == R_SECS && s_count < world->n_s)
	{
		...
		world->sec[s_count].ceil = 0;
		world->sec[s_count].fl_t = 0;
		world->sec[s_count].ceil_t = 0;
		world->sec[s_count].n_it = 0;
		...
		{
			ft_putstr("Incorrect fl or ceil for sector ");
			ft_putnbr(s_count);
			ft_putchar('\n');
			return (FAIL);
		}
...
unsigned				read_textures(t_media *media, t_section *section)
{
	...
	while (++i < section->n_files)
	{
		media->txtr[i].full_path = NULL;
		media->txtr[i].name = NULL;
		if (!(media->txtr[i].full_path = ft_strdup(section->tab[i])))
			return (FAIL);
...
unsigned				read_it_f(t_media *media, t_section *section)
{
	short 				i;

	if (!media || !section || !section->tab || section->n_files > MAX_ITEMFULL)
		return (FAIL);
	if (!(media->it_f = (t_it_f *)ft_memalloc(sizeof(t_it_f) * section->n_files)))
		return (FAIL);
	media->n_itf = section->n_files;
	ft_bzero(media->it_f, sizeof(t_it_f) * section->n_files);
	i = -1;
	while (++i < section->n_files)
	{
		media->it_f[i].full_path = NULL;
		media->it_f[i].name = NULL;
		if (section->tab[i] && !(media->it_f[i].full_path = ft_strdup(section->tab[i])))
			return (FAIL);
		if (section->names[i] && !(media->it_f[i].name = ft_strdup(section->names[i])))
			return (FAIL);
		if (section->extra)
			media->it_f[i].type = section->extra[i];
		else
			media->it_f[i].type = 0;
	}
	return (SUCCESS);
...
{
	unsigned			(*update_section[])(t_media*, t_section*) = { read_levels, read_textures, read_it_f, read_sounds, read_fonts };
...
void					free_media(t_media *media)
{
	...
	if (media->txtr)
	{
		...
		while (++i < media->n_t)
		{
			if (media->txtr[i].full_path)
				free(media->txtr[i].full_path);
	...
	if (media->it_f)
	{
		...
		while (++i < media->n_itf)
		{
			...
			if (media->it_f[i].full_path)
			{
				free(media->it_f[i].full_path);
				media->it_f[i].full_path = NULL;
			}
		}
		free(media->it_f);
