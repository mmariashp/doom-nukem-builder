...
void					render_texture_icons(t_btn *btn, t_sdl *sdl, int n_btn, t_texture *txtr)
{
	...
	while (i < n_btn)
	{
		...
		if (txtr[i].name)
			write_txt(txtr[i].name, sdl, (t_txtb){ text, TRUE, 0 });
		i++;
	}
}
...
void					r_textures(t_sdl *sdl, t_grid *grid, t_media *media, t_prog *prog)
{
    ...
    render_texture_icons(prog->modes[prog->m_id].btn, sdl, prog->modes[prog->m_id].n_btn, media->txtr);
